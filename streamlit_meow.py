...
def visualize_predictions(image, results):
    ...
    return yolo_labels  # Return YOLO labels for saving
...
def main():
    ...
    if uploaded_file is not None:
        ...
        yolo_labels = visualize_predictions(image, results)

        # Save the original image and YOLO labels to Google Drive
        ...
        original_filename = f"original_image_{current_time}.png"
        labels_filename = f"predicted_image_{current_time}.txt"

        # Save the original uploaded image to Google Drive
        original_image_bytes = io.BytesIO()
        image.save(original_image_bytes, format='PNG')
        original_image_bytes.seek(0)
        save_file_to_drive(service, IMAGE_FOLDER_ID, original_image_bytes, original_filename, 'image/png')
